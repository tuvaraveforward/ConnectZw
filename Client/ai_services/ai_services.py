...
Each public function first tries the configured AI microservice
(AI_MICROSERVICE_URL in settings).  If the URL is not set, or the
remote call fails, the built-in rule-based logic runs as a fallback.
"""
...
_TIMEOUT = 5  # seconds for every outbound HTTP call


# ── Internal helpers ──────────────────────────────────────────────────────────

def _ms_url():
    """Return the base URL of the AI microservice, or empty string."""
    return getattr(settings, "AI_MICROSERVICE_URL", "").rstrip("/")


def _ms_headers():
    return {"X-API-Key": getattr(settings, "AI_API_KEY", ""), "Content-Type": "application/json"}


def _post(path, payload):
    """POST to the microservice.  Returns parsed JSON or raises."""
    url = f"{_ms_url()}{path}"
    r = requests.post(url, json=payload, headers=_ms_headers(), timeout=_TIMEOUT)
    r.raise_for_status()
    return r.json()
...
def get_ai_recommendation(payload):
    """
    Return a product recommendation dict.
    Tries the microservice at POST /recommend first.
    """
    if _ms_url():
        try:
            data = _post("/recommend", payload)
            return data.get("recommendation", data)
        except Exception as exc:
            logger.warning("AI microservice /recommend failed: %s — using fallback", exc)
...
def get_chat_response(message, client_name=None):
    """
    Return an HTML chat reply string.
    Tries the microservice at POST /chat first.
    """
    if _ms_url():
        try:
            data = _post("/chat", {"message": message, "client_name": client_name or ""})
            return data.get("reply", "")
        except Exception as exc:
            logger.warning("AI microservice /chat failed: %s — using fallback", exc)
    ...
    if any(w in msg for w in category_triggers) and search_terms:
        ...
        return (
            ...
            f"Try a different keyword, or call our helpdesk at <strong>{HELPDESK_PHONE}</strong>."
        )
...
def check_fraud(payload):
    """
    Return a fraud decision dict: { decision, score, reason }.
    Tries the microservice at POST /fraud-check first.
    """
    if _ms_url():
        try:
            data = _post("/fraud-check", payload)
            return {
            ...
            logger.warning("AI microservice /fraud-check failed: %s — using fallback", exc)
    ...
    if any(k in merchant for k in ("fraud", "scam", "suspicious")):
        return {"decision": "BLOCKED", "score": 0.90, "reason": "Suspicious merchant name"}
...
def _extract_location_term(msg):
    for phrase in (" in ", " at ", " near ", " around "):
        if phrase in msg:
            return " ".join(msg.split(phrase)[-1].strip().split()[:3])
    return None


def _search_locations(term):
    return Location.objects.filter(
        Q(city__icontains=term) | Q(province__icontains=term) | Q(location_name__icontains=term),
        is_active=True,
